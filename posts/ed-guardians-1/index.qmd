...
dbscan_parameters = dict(
    eps=200,        # < 120 lightyears from neighbours
    min_samples=3   # at least 3 systems in a cluster
)
...
with open(guardian_outliers_json_pins, 'wt') as of:
    json.dump(dict(markers=[
            dict(
                pin='red',
                text=row[1]['System Name'],
                **{c:row[1][c] for c in ['x','y','z']}
            )
...
{embed_astro(guardian_clusters_json_pins)}
